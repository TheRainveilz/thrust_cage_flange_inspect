...
# ---------- 9. 调试 / 存图 ----------
PRINT_DEBUG = False                    # 打印每孔轮廓计数、有效压痕数、判定结果
SAVE_NG_IMAGE = False                  # NG 样本本地保存
SAVE_OK_IMAGE = False                 # OK 样本也保存(追溯用)
SAVE_OVERLAY = False                   # 保存时叠加检测结果(孔/ROI/拐角/环) 便于现场看图排查
...
def fit_circle_robust(pts: np.ndarray, iters: int, tol_ratio: float,
                      tol_min: float, min_pts: int) -> Optional[Tuple[float, float, float, int]]:
    return bcorner.fit_circle_robust(pts, iters, tol_ratio, tol_min, min_pts)
...
def feature_b_corner_marks(gray: np.ndarray, clahe_only: np.ndarray, hx: float, hy: float, r: float,
    ...
    for ang, dist_ratio in corner_spec:
        ...
        cand_np = np.array(cand, dtype=np.float64)
        # 全部圆度、多阈值、去重逻辑全部下沉到C++ bcorner模块
        dynamic_gate_val = 0.32 if dynamic_roi else MARK_CENTER_GATE
        res = bcorner.select_best_candidate(
            cand_np,
            win,
            win_enhanced,
            half,
            ...
            MARK_CENTER_GATE,
            dynamic_gate_val,
            MARK_CIRCULARITY_MIN,
            MARK_MASK_RATIO,
            MARK_MIN_AREA_RATIO,
            MARK_DEDUP_DIST_RATIO,
            MARK_MAX_CANDIDATES,
            np.array(MARK_THRESH_PCTS, dtype=np.float64)
        )
        best = res
        ...
        if res.accepted:
            ...
            valid +=1
        if IS_DEBUG_DETAIL:
            rec["circ"] = round(res.circ,3)
            rec["circ_raw"] = round(res.circ_raw,3)
            rec["circ_enhanced"] = round(res.circ_enhanced,3)
            rec["center_offset"] = round(res.center_offset,3)
            rec["radius_ratio"] = round(res.radius_ratio,3)
            rec["shape_ok"] = bool(res.shape_ok)
            rec["dynamic_roi"] = bool(dynamic_roi)
            rec["r"] = round(res.radius_ratio,3)
            rec["mark_cx"] = px + (res.bx - half)
            rec["mark_cy"] = py + (res.by - half)
            rec["mark_r"] = float(res.br)
        details.append(rec)
...
def inspect(bgr: np.ndarray, name: str = "") -> Tuple[InspectResult, np.ndarray]:
    ...
    for (x0, y0, r0) in cand:
        ...
        got = bcorner.refine_hole(
            work,
            float(x0), float(y0), float(r0),
            cos_t, sin_t,
            REFINE_SCAN_BAND[0], REFINE_SCAN_BAND[1],
            REFINE_RADIUS_STEP_PX,
            REFINE_INNER_BAND, REFINE_LAND_BAND,
            REFINE_MIN_CONTRAST,
            REFINE_EDGE_START,
            REFINE_MIN_EDGE_PTS,
            REFINE_ITERS,
        ...
        cx, cy, r, contrast, flag = got

        if flag == 1:
            refined.append((cx, cy, r, contrast))
    ...
    for hole in holes[:n_check]:
        ...
        raw_cnt, ring_cnt, ring_radii = bcorner.feature_a_ring_contours(
            gray, hole.cx, hole.cy, hole.r,
            RING_ROI_RATIO,
            RING_MASK_RATIO,
            RING_BAND[0], RING_BAND[1],
            np.array(RING_THRESH_PCTS, dtype=np.float64),
            RING_MIN_CONTOUR_PTS,
            RING_MAX_RADIAL_STD,
            RING_MIN_ANGLE_COVER,
            RING_CLUSTER_GAP,
            RING_CLUSTER_MIN_HITS,
            CLAHE_CLIP,
            CLAHE_GRID[0], CLAHE_GRID[1],
            GAUSS_BLUR_K
        )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        for idx, (name, bgr) in enumerate(source.frames()):
            ...
            # ====== 只有--print-detail开启时，才打印完整孔、拐角明细 ======
            if PRINT_DEBUG:
                print_result(res)
