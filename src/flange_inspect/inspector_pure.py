...
COLLAR_GATE = True   # 2026-09-22 验收锁定: 全量 975 张, 正面图片级 340 零代价, 反面逃逸 0
# 2026-09-22 高阈值扫描(离线, 关闸跑一遍后内存重算 11 档)锁定 0.12:
#   正面图片级召回在 0.05~0.12 之间**恒定 340/352, 一张不掉**(免费段); 0.15 起才开始掉。
#   反面 A 孔级随阈值单调下降: 0.05=4.1% → 0.06=3.5% → 0.10=2.6% → 0.12=2.4%
#                              → 0.20=1.4% → 0.30=0.6% → 0.50=0.0%。
#   要压到 0% 得开到 0.50, 但那时正面召回崩到 53/352(15%), 且反面逃逸全程是 0(B 一道闸独撑)
#   —— 压到 0% 换不回一张逃逸裕度, 纯亏。故取免费段最紧的一档 0.12: 340 召回不变,
#   反面 A 孔级从 0.05 的 4.1% 再降到 2.4%, 白捡地加厚第二道闸的独立性。
COLLAR_DARKFRAC_MIN = 0.12  # 免费段(正面 340 不掉)内最紧档; 反面 A 孔级压到 2.4%。上限见上方扫描注释
